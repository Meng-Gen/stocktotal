<?php
    ...
    class StocktotalDatabase {
        ...
        public function query_current_and_rapid_ratio($stock_code) {
            return $this->db_impl->query_current_and_rapid_ratio($stock_code);
        }
        
        public function query_nonoperating_income($stock_code) {
            return $this->db_impl->query_nonoperating_income($stock_code);
        }
        
        public function query_long_term_investments($stock_code) {
            return $this->db_impl->query_long_term_investments($stock_code);
        }
        
        public function query_operating_income($stock_code) {
            return $this->db_impl->query_operating_income($stock_code);
        }
        
        public function query_accumlated_income_yoy($stock_code) {
            return $this->db_impl->query_accumlated_income_yoy($stock_code);
        }
        
        public function query_profit_margin($stock_code) {
            return $this->db_impl->query_profit_margin($stock_code);
        }
        
        public function query_cash_flow($stock_code) {
            return $this->db_impl->query_cash_flow($stock_code);
        }
        
        public function query_ccc($stock_code) {
            return $this->db_impl->query_ccc($stock_code);
        }
        
        public function query_evaluation_index($stock_code) {
            return $this->db_impl->query_evaluation_index($stock_code);
        }
        
        public function query_stock_dividend($stock_code) {
            return $this->db_impl->query_stock_dividend($stock_code);
        }
        
        public function query_capital_structure_summary($stock_code) {
            return $this->db_impl->query_capital_structure_summary($stock_code);
        }
        
        public function query_expected_roe_range($stock_code) {
            return $this->db_impl->query_expected_roe_range($stock_code);
        }
        
        public function query_expected_price_range($stock_code) {
            return $this->db_impl->query_expected_price_range($stock_code);
        }
