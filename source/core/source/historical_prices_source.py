...
class HistoricalPricesSource():
    ...
    def __init__(self):
        ...
        self.URL_TEMPLATE = '''http://finance.yahoo.com/q/hp?s=%s.TW+Historical+Prices'''
        self.URL_DELTA_TEMPLATE = '''http://ichart.finance.yahoo.com/table.csv?s=%s.TW&amp;d=%02d&amp;e=%s&amp;f=%s&amp;g=d&amp;a=%02d&amp;b=%s&amp;c=%s&amp;ignore=.csv'''
    ...
    def source_delta(self, stock_code, begin_date, end_date):
        self.source_url_to_csv_delta(self.DELTA_DIR, stock_code, begin_date, end_date)
    ...
    def source_url_to_html(self, dest_dir, stock_code):
        ...
        url = self.__get_url(stock_code)
        dest_file = self.__get_filename(dest_dir, stock_code, 'html')
        self.__wget(url, dest_file)

    def source_url_to_csv_delta(self, dest_dir, stock_code, begin_date, end_date):
        if not os.path.exists(dest_dir):
            os.makedirs(dest_dir)
        url = self.__get_url_delta(stock_code, begin_date, end_date)
    ...
    def __get_url(self, stock_code):
        return self.URL_TEMPLATE % stock_code

    def __get_url_delta(self, stock_code, begin_date, end_date):
